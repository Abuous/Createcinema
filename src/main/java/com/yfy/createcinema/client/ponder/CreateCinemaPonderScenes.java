...
public final class CreateCinemaPonderScenes {
    ...
    public static void filmBurning(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(80);
        scene.addKeyframe();
        scene.overlay().showText(80).text("选择刻录画质并开始处理。画质决定成品的清晰度，刻录期间胶卷会留在刻录机中。")
    ...
    public static void projectorSetup(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        for (int z = 2; z <= 4; z++) {
            for (int y = 1; y <= 3; y++) {
                scene.world().setBlock(util.grid().at(5, y, z), screen(Direction.WEST), false);
            }
        }
        ...
        scene.overlay().showText(80).text("幕布要朝向放映机摆放，薄板平面应正对投影方向，投影才会铺满整块幕布。")
                .colored(PonderPalette.BLUE).pointAt(util.vector().centerOf(6, 2, 4)).placeNearTarget();
        scene.idle(100);
        scene.addKeyframe();
        scene.overlay().showText(80).text("放映机需要有效转速。64 rpm 为 1 倍速，过载或 0 rpm 会停止播放。")
                .colored(PonderPalette.MEDIUM).pointAt(util.vector().topOf(projector)).placeNearTarget();
        scene.idle(90);
        scene.addKeyframe();
        scene.overlay().showText(70).text("右侧升级槽每槽仅安装一张升级卡。装入显示升级卡后，放映机改为在幕布上输出显示连接器文本。")
    ...
    public static void networkAudio(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        for (int z = 2; z <= 4; z++) {
            for (int y = 1; y <= 3; y++) {
                scene.world().setBlock(util.grid().at(6, y, z), screen(Direction.WEST), false);
            }
        }
        ...
        scene.idle(90);
        scene.overlay().showText(80).text("手持安山岩或黄铜机壳右键线缆即可套上机壳，相邻机壳会自动融合成一整段外壳。")
                .colored(PonderPalette.BLUE).pointAt(util.vector().centerOf(2, 1, 4)).placeNearTarget();
        scene.idle(90);
        scene.addKeyframe();
        ...
        scene.overlay().showText(90).text("音箱音量由红石强度 0 到 15 线性控制。0 是静音，15是最大音量。")
    ...
    private static BlockState screen(Direction facing) {
        return ModRegistry.SCREEN.get().defaultBlockState().setValue(ScreenBlock.FACING, facing);
    }
